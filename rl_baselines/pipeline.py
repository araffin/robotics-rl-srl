...
def main():
    ...
    parser.add_argument('--srl-model', type=str, default='all',
                        choices=['all', "autoencoder", "ground_truth", "srl_priors", "supervised", "pca", "vae", "joints", "joints_position", "raw_pixels"],
    ...
    if args.srl_model == "all":
        models = ["autoencoder", "ground_truth", "srl_priors", "supervised", "pca", "vae", "joints", "joints_position", "raw_pixels"]
    else:
    ...
    for model in models:
        for env in envs:
            for seed_idx in range(len(SEEDS)):
                ...
                train_args.extend(['--seed', str(SEEDS[seed_idx]), '--algo', args.algo, '--env', env, '--num-timesteps', str(args.num_timesteps)])
